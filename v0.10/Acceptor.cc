...
void Acceptor::HandleRead() {//这个sockFd就是lisenFd，没有用
    cout << "[INFO] Acceptor::HandleRead listen from: " << listenFd_ << endl;
    int connFd;
    sockaddr_in cliAddr;
    socklen_t cliLen;
    ...
    if (connFd == -1) {
        if (errno == EMFILE) {
            ...
            if ((connFd = accept(listenFd_, NULL, NULL)))
                ERR_EXIT("[ERRO] Acceptor::HandleRead accept");
            close(connFd);
...
int Acceptor::CreateAndListen() {
    ...
    if ((listenFd_ = socket(PF_INET, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, IPPROTO_TCP)) < 0)
        ERR_EXIT("[ERRO] Acceptor::CreateAndListen socket");
    ...
    if (bind(listenFd_, (sockaddr*)&srv_addr, sizeof(srv_addr)) < 0)
        ERR_EXIT("[ERRO] Acceptor::CreateAndListen bind");
